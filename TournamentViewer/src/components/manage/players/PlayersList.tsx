...
export default function PlayersList() {
    ...
    const addToTeam = (playerId: number, teamId: number) => {
        try {
            axios.post(`players/${playerId}/assignToTeam/${teamId}`).then((response) => {
    ...
    const removeFromTeam = (playerId: number) => {
        try {
            axios.post(`players/${playerId}/removeFromTeam`).then(() => {
                setPlayers(players.map((p) => p.id === playerId ? {...p, teamId: undefined} : p));
            ...
            <div className="flex flex-col justify-start gap-3">
                ...
                <div className="flex flex-row gap-5">
                    ...
                    <div>
                        ...
                        {selectedPlayerId >= 0 && (
                            <PlayerItem teams={teams} player={getSelectedPlayer() as Player} addToTeam={addToTeam}
                                        removeFromTeam={removeFromTeam} createTeam={createTeam}
...
function PlayerItem({player, teams, addToTeam, removeFromTeam, createTeam, deleteTeam}: {
    player: Player,
    teams: Team[],
    ...
    return (
        <div className={'flex flex-col gap-2'}>
            ...
            <div className={'flex flex-row gap-2 items-center'}>
                ...
                <button onClick={createTeam}><FontAwesomeIcon icon={faPlus}/></button>
